...
class JadwalController extends Controller
{
	...
	public function index_jadwal(Request $request)
	{
		...
		$data['hari'] = Hari::all()->pluck('hari', 'id');

		// $query = Jadwal::query()->where('id_semester', get_semester('active_semester_id'))->orderBy(Guru::select('nama')->whereColumn('guru.id', 'jadwal.id_guru'));
		$query = DB::table('jadwal')
			->select('jadwal.*', 'g.nama', 'm.mapel')
			->Join('guru as g', 'jadwal.id_guru', '=', 'g.id')
			->Join('mapel as m', 'jadwal.id_mapel', '=', 'm.id')
			->where('jadwal.id_semester', session()->get('active_semester')['id']);
		if ($request->has('search')) {
			$search = $request->get('search');
			$query->where('g.nama', 'like', "%$search%");
			$query->Orwhere('m.mapel', 'like', "%$search%");
		}
		$data['data'] = $query->paginate(20)->withQueryString();
		// $data['jam_pelajaran'] = Jampelajaran::all()->pluck('jam_pelajaran','id');

		$this->log($request, 'melihat halaman manajemen data ' . $this->title);
		return view('Jadwal::jadwal_index', array_merge($data, ['title' => $this->title]));
	}
