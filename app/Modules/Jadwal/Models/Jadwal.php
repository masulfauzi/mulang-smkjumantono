...
class Jadwal extends Model
{
    ...
    public static function get_jadwal_by_kelas_mapel($id_kelas, $id_mapel, $id_semester)
    {
        return DB::table('jurnal as j')
            ->select('e.nama', 'c.kelas', 'd.semester', 'm.mapel', 'd.id as id_semester', 'j.id as id_jadwal', 'j.tgl_pembelajaran', 'j.materi', 'j.catatan', 'j.jam_mulai', 'j.jam_selesai')
            ->join('mapel as m', 'j.id_mapel', '=', 'm.id')
            ->join('kelas as c', 'j.id_kelas', '=', 'c.id')
            ->join('semester as d', 'j.id_semester', '=', 'd.id')
            ->join('guru as e', 'j.id_guru', '=', 'e.id')
            ->where('j.id_kelas', $id_kelas)
            ->where('j.id_mapel', $id_mapel)
